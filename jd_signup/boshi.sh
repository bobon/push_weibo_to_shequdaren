...
if [ "$a" = "false" ]; then
echo -e "\n$(date +"%x %X %N  %s") 赵签到博世"
t=$(curl -sS -k 'https://api.m.jd.com/api?appid=interCenter_shopSign&t=1609588867000&loginType=2&functionId=interact_center_shopSign_signCollectGift&body=\{%22token%22:%2237B57F259CEA113FC7F0D636B3DD68FF%22,%22venderId%22:1000002423,%22activityId%22:10232978,%22type%22:56,%22actionType%22:7\}&jsonp=jsonp1004' \
  ...
  if [ $(echo "$t" | jq '.success' | grep 'true'>/dev/null;echo $?) -eq 0 ]; then
  	a=true
  elif [ $(echo "$t" | jq '.msg' | egrep '你已经参加过该活动啦|当天只允许签到一次'>/dev/null;echo $?) -eq 0 ]; then
  	a=true
